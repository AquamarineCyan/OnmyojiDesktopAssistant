...
class JieJieTuPoGeRen(JieJieTuPo):
    ...
    def title(self):
        ...
        while 1:
            if self.judge_scene(f"{self.picpath}/title.png", "结界突破"):
                while 1:
                    if self.judge_scene(f"{self.picpath}/fangshoujilu.png", "个人突破"):
                        return True
                    else:
                        time.sleep(1)
            ...
            if self.list_xunzhang.count(i):
                ...
                for j in range(1, self.list_xunzhang.count(i) + 1):
                    ...
                    if x != 0 and y != 0:
                        ...
                        continue
                    self.fighting_tupo(
                        self.tupo_geren_x[(k + 2) % 3 + 1], self.tupo_geren_y[(k + 2) // 3])
                    # 待优化，利用时间戳的间隔判断
                    # time.sleep(4)
...
class JieJieTuPoYinYangLiao(JieJieTuPo):
    ...
    def title(self):
        ...
        while 1:
            if self.judge_scene(f"{self.picpath}/title.png", "结界突破"):
                while 1:
                    if self.judge_scene(f"{self.picpath}/tupojilu.png", "阴阳寮突破"):
                        return True
                    else:
                        time.sleep(1)
                        self.judge_click(
                            f"{self.picpath}/yinyangliao.png", dura=0.5)
                        time.sleep(4)
            elif flag_title:
                flag_title = False
                log.warn("请检查游戏场景", True)
    ...
    def fighting(self):
        ...
        while 1:
            ...
            if x == 0 or y == 0:
                ...
                self.fighting_tupo(self.tupo_yinyangliao_x[(
                    i + 1) % 2 + 1], self.tupo_yinyangliao_y[(i + 1) // 2])
                if self.result():
                    # 胜利
                    flag = 1
                else:
